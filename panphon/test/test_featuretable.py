...
class TestFeatureTable(unittest.TestCase):
    ...
    def test_match_pattern(self):
        self.assertTrue(
            self.ft.match_pattern([{"voi": -1}, {"voi": 1}, {"voi": -1}], "pat")
        )
    ...
    def test_word_to_vector_list_aspiration(self):
        self.assertNotEqual(
            self.ft.word_to_vector_list("pʰ"), self.ft.word_to_vector_list("p")
        )

    def test_word_to_vector_list_aspiration_xsampa(self):
        self.assertNotEqual(
            self.ft.word_to_vector_list("p_h", xsampa=True),
            self.ft.word_to_vector_list("p", xsampa=True),
        )

    def test_word_to_vector_list_aspiration_xsampa_len(self):
        self.assertEqual(len(self.ft.word_to_vector_list("p_h", xsampa=True)), 1)

    def test_word_to_vector_list_numeric_tones_len(self):
        self.assertEqual(
            self.ft.word_to_vector_list("i²⁴"), self.ft.word_to_vector_list("i˨˦")
        )
...
class TestIpaRe(unittest.TestCase):
    ...
    def test_word_vector_list_english(self):
        input_word = "maɪkɫbɛniatɡimeɪldɑtkʰɔm"  # michaelbennie@gmail.com
        ...
        input_word2 = "kʰalɪkʰəʊkʰats"  # michaelbennie@gmail.com
    ...
    def test_word_vector_list_chinese(self):
        input_word = "tɕi˥pʰu˧˥tʰɑʊ˧˥pu˥˩tʰu˩˨pʰu˧˥tʰɑʊ˧˥pʰi˧˥pu˥˩tɕi˥pʰu˧˥tʰɑʊ˧˥tɑʊ˥˩tʰu˩˨pʰu˧˥tʰɑʊ˧˥pʰi˧˥"
        vector_list = self.ft.word_to_vector_list(input_word, numeric=True)
        new_word_list = self.ft.vector_list_to_word(vector_list)
        self.assertEqual(input_word, new_word_list)
...
class TestXSampa(unittest.TestCase):
    ...
    def test_affricates(self):
        self.assertNotEqual(
            self.ft.word_to_vector_list("tS", xsampa=True),
            self.ft.word_to_vector_list("t S", xsampa=True),
        )
